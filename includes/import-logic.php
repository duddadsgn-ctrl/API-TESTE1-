...
function vit_import_property( $api_url, $api_key, $property_code = '' ) {
    ...
    if ( ! empty( $property_code ) ) {
        ...
        $response = vit_call_api( $api_url, $endpoint, $api_key, $post_fields );
        ...
        // Busca o primeiro imóvel da lista
        ...
        $response_list = vit_call_api( $api_url, $endpoint_list, $api_key, $post_fields );
        ...
        $response_details = vit_call_api( $api_url, $endpoint_details, $api_key, $post_fields_details );
...
/**
 * Faz a chamada para a API Vista usando POST, com chave na URL e parâmetros no corpo JSON.
 * Esta é a forma correta para TODOS os endpoints, conforme a documentação.
 */
function vit_call_api( $base_url, $endpoint, $api_key, $post_fields = [] ) {
...
// Cole a função vit_call_api acima no lugar das antigas funções de chamada de API.

function vit_get_or_create_property_post( $vista_code, &$log ) {
    $args = [
        'post_type' => 'imoveis', 'post_status' => 'any', 'meta_key' => '_vista_codigo',
        'meta_value' => $vista_code, 'posts_per_page' => 1, 'fields' => 'ids',
    ];
    ...
    if ( $query->have_posts() ) {
        ...
        $post_id = wp_insert_post( [
            'post_type' => 'imoveis', 'post_status' => 'publish', 'post_title' => 'Imóvel ' . $vista_code,
        ] );
