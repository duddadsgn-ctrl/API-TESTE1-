...
function vit_import_property( $api_url, $api_key, $property_code = '' ) {
    ...
    if ( ! empty( $property_code ) ) {
        ...
        // Busca o primeiro imóvel da lista (usa POST com JSON no body)
        $log[] = 'Buscando lista de imóveis para pegar o primeiro...';
        $endpoint_list = '/imoveis/listar';
        $post_fields = [ 'paginacao' => [ 'pagina' => 1, 'quantidade' => 1 ] ];
        $response_list = vit_call_api_post( $api_url, $endpoint_list, $api_key, $post_fields );
        ...
        $response_details = vit_call_api_get( $api_url, $endpoint_details, $api_key, $query_params_details );
...
/**
 * Faz a chamada para a API Vista usando GET (para /detalhes).
 */
function vit_call_api_get( $base_url, $endpoint, $api_key, $query_params = [] ) {
...
/**
 * Faz a chamada para a API Vista usando POST (para /listar).
 */
...
function vit_handle_api_response( $response ) {
    ...
    if ( $http_code === 401  ) {
        return new WP_Error( 'api_auth_error', "API retornou erro 401 (Não Autorizado). Verifique se a Chave API está correta." );
    }
...
// O restante do arquivo (as funções vit_get_or_create_property_post, vit_update_property_fields, etc.) permanece exatamente o mesmo.
// Cole as funções acima no lugar das antigas funções de chamada de API.
...
function vit_update_property_fields( $post_id, $property_data, &$log ) {
    ...
    $meta_map = [
        '_vista_codigo' => 'Codigo', 'codigo' => 'Codigo', 'codigo_corretor' => 'CodigoCorretor',
        'bairro' => 'Bairro', 'cidade' => 'Cidade', 'uf' => 'UF', 'latitude' => 'Latitude', 'longitude' => 'Longitude',
        'status' => 'Status', 'finalidade' => 'Finalidade', 'categoria' => 'Categoria', 'moeda' => 'Moeda',
        'dormitorios' => 'Dormitorios', 'suites' => 'Suites', 'banheiros' => 'BanheiroSocialQtd', 'vagas' => 'Vagas',
        'area_total' => 'AreaTotal', 'area_privativa' => 'AreaPrivativa', 'valor_venda' => 'ValorVenda',
        'valor_locacao' => 'ValorLocacao', 'valor_iptu' => 'ValorIptu', 'valor_condominio' => 'ValorCondominio',
    ];
    ...
    if ( ! empty( $property_data['Latitude'] ) && ! empty( $property_data['Longitude'] ) ) {
        update_post_meta( $post_id, 'mapa', "{$property_data['Latitude']},{$property_data['Longitude']}" );
    }

    $feature_arrays = [
        'caracteristicas' => 'Caracteristicas', 'infraestrutura'  => 'InfraEstrutura', 'imediacoes' => 'Imediacoes',
    ];
...
function vit_process_property_images( $post_id, $property_data, &$log ) {
    ...
    if ( isset( $photos[0]['Ordem'] ) ) {
        usort( $photos, fn($a, $b) => $a['Ordem'] <=> $b['Ordem'] );
    ...
    if ( ! empty( $gallery_ids ) ) {
        ...
        update_post_meta( $post_id, 'galeria', $gallery_ids ); // Depuração
    }
}
...
function vit_sideload_image( $file_url, $post_id, $desc ) {
    $args = [
        'post_type' => 'attachment', 'post_status' => 'inherit', 'post_parent' => $post_id,
    ...
    if ( ! $file_array['name'] ) {
        ...
        return new WP_Error( 'image_sideload_failed', 'Não foi possível determinar o nome do arquivo.' );
    }
